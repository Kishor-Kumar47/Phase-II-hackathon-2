...
async def get_tasks(
    ...
    tasks = TaskService.get_tasks_by_user(session, 1)
    return tasks
...
async def create_task(
    ...
    task_data_dict = task_data.model_dump()
    task_data_dict['user_id'] = 1
    task = TaskService.create_task(session, TaskBase(**task_data_dict))
    return task

@router.get("/1/tasks/{id}")
async def get_task(
    id: int = Path(..., description="ID of task to retrieve"),
    session: Session = Depends(get_session)
):
    task = TaskService.get_task_by_id(session, id, 1)
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
    return task

@router.put("/1/tasks/{id}")
async def update_task(
    ...
    task_data_dict = task_data.model_dump()
    task_data_dict['user_id'] = 1
    updated_task = TaskService.update_task(session, id, 1, TaskBase(**task_data_dict))
    if not updated_task:
        raise HTTPException(status_code=404, detail="Task not found")
    return updated_task

@router.delete("/1/tasks/{id}")
async def delete_task(
    id: int = Path(..., description="ID of task to delete"),
    session: Session = Depends(get_session)
):
    success = TaskService.delete_task(session, id, 1)
    if not success:
        raise HTTPException(status_code=404, detail="Task not found")
    return {"message": "Task deleted successfully"}

@router.patch("/1/tasks/{id}/complete")
async def complete_task(
    id: int = Path(..., description="ID of task to update"),
    is_completed: bool = Body(..., embed=True),
    session: Session = Depends(get_session)
):
    updated_task = TaskService.complete_task(session, id, 1, is_completed)
    if not updated_task:
        raise HTTPException(status_code=404, detail="Task not found")
    return updated_task
